<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input placeholder="请输入你的登录密码" type="password" v-model="password" name="password" v-validate="{required: true, regex: /^[0-9A-Za-z]{8,20}$/}" :class="{invalid: errors.has('password')}"/>
        <span class="error-msg">{{errors.first('password')}}</span>
      </div>
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请确认你的登录密码" type="password" v-model="password2" name="password2" v-validate="{required: true, is:password}" :class="{invalid: errors.has('password2')}"/>
      ...
      <div class="controls">
        ...
        <span class="error-msg">错误提示信息</span> -->
        <input type="checkbox" v-model="agree" name="agree" v-validate="{required:true, checkedRule:true}" :class="{invalid: errors.has('agree')}"/>
        <span>同意协议并注册《尚品汇用户协议》</span>
...
export default {
  ...
  methods: {
    ...
    async userRegister() {
      ...
      try{
        (phone&&code&&password&&password==password2) && await this.$store.dispatch('userRegister', {phone, code, password});
        //注册成功跳转登录
