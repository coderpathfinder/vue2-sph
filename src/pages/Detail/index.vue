        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in attrList" :key="attr.id">
                <dt class="title">{{attr.saleAttrName}}</dt>
                <dd changepirce="40" 
                :class="{active:attrValue.isChecked==1}" 
                v-for="attrValue in attr.spuSaleAttrValueList" 
                @click="changeSaleAttr(attrValue, attr.spuSaleAttrValueList)"
                :key="attrValue.id">{{attrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" @change="skuNumChangeHandler" v-model="skuNum" class="itxt">
                <a href="javascript:" @click="skuNum++" class="plus">+</a>
                <a href="javascript:" @click="subSkuNum" class="mins">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    changeSaleAttr(saleAttrValue, attr) {
      attr.forEach(item => {
        item.isChecked = '0';
      });
    ...
    async addShopCart() {
      try{
        ...
        //成功后向sessionStorage中存储商品详细信息，并进行路由跳转
